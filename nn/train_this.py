...
def validataion():
    ...
    MiP, MiR, MiF, P_NUM, T_NUM = micro_score(outputs, test_Y)
...
with tf.Session() as sess:
    ...
    for epoch in range(epoch_num_cnn):
        ...
        for iter, idx in enumerate(range(0, Reader.train_size, batch_size)):
            batch_X = Reader.mapping_sequence(Reader.train_X[idx:idx + batch_size])
            batch_Y = Reader.mapping_multi_hot(Reader.train_Y[idx:idx + batch_size])
