...
class TextCNN(object):
    def __init__(self, embeddings):
        ...
        with tf.name_scope("Output_Part"):
            ...
            self.loss_cnn = -tf.reduce_mean(tf.multiply(self.y, tf.log(tf.clip_by_value(logits, 1e-10, 1.0))))

            gamma = 1
            lfl = -tf.reduce_mean(tf.multiply(self.y, tf.log(tf.clip_by_value(logits, 1e-10, 1.0))))
