...
class TextCNN(object):
    def __init__(self, embeddings):
        ...
        with tf.name_scope("Output_Part"):
            ...
            #     tf.multiply(self.y, tf.log(tf.clip_by_value(logits, 1e-10, 1.0))))  # wrong loss,but good result 0.427

            gamma = 1
            lfl = -tf.reduce_mean(
                self.y * tf.pow(1 - logits, gamma) * tf.log(tf.clip_by_value(logits, 1e-10, 1.0)) + (
