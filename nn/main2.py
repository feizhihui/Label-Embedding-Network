...
Reader = data_input2.data_master()
...
def validataion(model_prediction):
    ...
    outputs = []
    for i in range(0, len(Reader.test_labels), batch_size):
        test_sentences_batch = Reader.test_sentences[i:i + batch_size]
        output = sess.run(model_prediction,
                          feed_dict={model.x: test_sentences_batch, model.dropout_keep_prob: 1.0})
        outputs.append(output)

    outputs = np.concatenate(outputs, axis=0)

    MiP, MiR, MiF, P_NUM, T_NUM = micro_score(outputs, Reader.test_labels)
    ...
    # MaP, MaR, MaF = macro_score(outputs, Reader.test_labels)
...
with tf.Session() as sess:
    ...
    for epoch in range(epoch_num_cnn):
        Reader.shuffle()
        for iter, (batch_x, batch_y) in enumerate(Reader.batch_iter(batch_size)):
