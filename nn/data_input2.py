# encoding=utf-8
import pickle
import numpy as np
import random

time_steps = 700
class_num = 6984
training_rate = 0.9
shuffle = False


class data_master:
    def __init__(self):
        # with open('../PKL2/lookup_matrix.pkl', 'rb') as file:
        #     self.embeddings = pickle.load(file)

        self.embeddings = np.load('../PKL/lookup_matrix.npy')
        with open('../PKL/lookup_dict.pkl', 'rb') as file:
            self.word_dict = pickle.load(file)
        with open('../PKL/code_dict.pkl', 'rb') as file:
            self.code_dict = pickle.load(file)
        with open('../PKL/all_text.pkl', 'rb') as file:
            all_text = pickle.load(file)
        with open('../PKL/all_code.pkl', 'rb') as file:
            all_code = pickle.load(file)

        data_sentences = np.zeros([len(all_text), time_steps], dtype=np.int32)
        for i, sample in enumerate(all_text):
            for j, word in enumerate(sample[:time_steps]):
                token = self.word_dict[word]
                data_sentences[i, j] = token

        # ==================
        data_labels = self.change_multi_hot(all_code)
        data_sentences = np.array(data_sentences, dtype=np.float32)
        data_labels = np.array(data_labels)
        if shuffle:
            data_sentences, data_labels = self.shuffle(
                data_sentences,
                data_labels)

        split_line = int(training_rate * len(data_sentences))
        self.train_X = data_sentences[:split_line]
        self.train_Y = data_labels[:split_line]
        self.train_size = split_line

        self.test_X = data_sentences[split_line:]
        self.test_Y = data_labels[split_line:]

    def batch_iter(self, batch_size):
        for iter, index in enumerate(range(0, len(self.train_Y), batch_size)):
            batch_x = self.train_X[index:index + batch_size]
            batch_y = self.train_Y[index:index + batch_size]
            yield batch_x, batch_y

    def shuffle(self, data_sentences=None, data_labels=None):
        if data_sentences is None:
            # bug
            permutation = np.random.randint(self.train_size, size=self.train_size)
            self.train_X = self.train_X[permutation]
            self.train_Y = self.train_Y[permutation]

            # ground-truth code
            # indices = list(range(len(self.train_Y)))
            # random.shuffle(indices)
            # self.train_X = self.train_X[indices]
            # self.train_Y = self.train_Y[indices]

        else:
            indices = list(range(len(data_sentences)))
            random.shuffle(indices)
            data_sentences = data_sentences[indices]
            data_labels = data_labels[indices]

            return data_sentences, data_labels

    def change_multi_hot(self, batch_y):

        batch_Y_matrix = np.zeros([len(batch_y), class_num], dtype=np.int32)
        for i, code_set in enumerate(batch_y):
            for code in code_set:
                code = self.code_dict[code]
                batch_Y_matrix[i, code] = 1

        return batch_Y_matrix


if __name__ == '__main__':
    master = data_master()
    print(master.test_X)
    print(master.test_Y)
    print(len(master.test_Y))
